...
- Anthropic native `/v1/messages` is supported for streaming and non-streaming requests. Gemini native protocols are not supported yet; Gemini routed through Google's OpenAI-compatible endpoint is covered by the existing OpenAI-compatible WAF path. See [docs/protocol-support.md](docs/protocol-support.md) for the current support matrix.
